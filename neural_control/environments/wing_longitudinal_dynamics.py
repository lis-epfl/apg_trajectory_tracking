...
def long_dynamics(state, action, dt):
    ...
    u_dot = -w * q +(1/m) * (T + L * torch.sin(alpha) - D * torch.cos(alpha) - m * g * torch.sin(theta))
    w_dot = u * q -(1/m) * (L * torch.cos(alpha) - D * torch.sin(alpha) - m * g * torch.cos(theta))
    ...
    state_dot = torch.vstack((x_dot, h_dot, u_dot, w_dot, theta_dot, q_dot)).t()
